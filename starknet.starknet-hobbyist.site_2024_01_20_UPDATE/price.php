...
class index extends _api
{
    ...
    public function _echarts(): void
    {
        ...
        $query = match ($type) {
            1 => 'ROUND(AVG(zhuanzhang),2)',
            2 => 'ROUND(AVG(shangpin4),2)',
            default => 'ROUND(AVG(zhuanzhang),2)/ROUND(AVG(shangpin4),2)',
        };
...
GROUP BY
    ...
    public function _card(): void
    {
        ...
        $sql = "
SELECT
    ROUND(shangpin4,2) AS min_shangpin4,
    ROUND(zhuanzhang,2) AS min_zhuanzhang
FROM
    hangqing
WHERE
    ...
    public function _line()
    {
        ...
        foreach ($date as $item) {
            ...
            $data[] = [filter($item, $zhuanzhang), filter($item, $shangpin4)];
        }
...
<div class="main layui-form">
    <div class="price">
        ...
        <div class="tools">
            ...
            <div>
                <div class="chart-type">
                    ...
                    <input type="radio" name="type" value="0" title="Ethereum / Starknet">
                </div>
            </div>
        </div>
        ...
        <div class="desc" style="margin-bottom: 150px;text-align: center;color: #999999;position: relative">
            <span>You can click on Starknet or Ethereum to display a separate candlestick chart.</span>
            ...
            $.ajax({
                ...
                success: r => {
                    ...
                    if (r.data.min_shangpin4 !== '0.00') {
                        animate(0, r.data.min_shangpin4, $('.card').eq(0).find('.price span'));
                    } else {
                        $('.card').eq(0).find('.price span').text('0.00');
                    }

                    if (r.data.min_zhuanzhang !== '0.00') {
                        animate(0, r.data.min_zhuanzhang, $('.card').eq(1).find('.price span'));
                    } else {
                        $('.card').eq(1).find('.price span').text('0.00');
                    }
